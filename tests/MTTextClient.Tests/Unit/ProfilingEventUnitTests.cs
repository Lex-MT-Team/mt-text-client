...
/// <summary>
/// In-process coverage for the algorithm-profiling event bridge: the event
/// ring-buffer capacity, the field-preserving serialization the bridge depends
/// on, and the shape of the published `algo_profiling` event. No MTCore/network.
/// </summary>
public sealed class ProfilingEventUnitTests
{
    ...
    // does not silently evict other cores' events before a client polls.
    [Fact]
    [Trait("Category", "Unit")]
