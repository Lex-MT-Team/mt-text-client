...
for the shape of the trading-performance feed the client now exposes. The request and
event mappings were derived from assembly inspection in the 0.7.25589 migration
and are exercised locally by Static/Unit tests. They are not a claim of live-core
validation. `lib/vendor.json` pins the platform-specific assemblies restored
under `lib/<rid>/`.
...
| `ADD_GROUP` (new folder) | `AlgorithmFolderAddRequestData` | `folder` |
...
same set. The implemented algorithm and single-folder operations, including
START_ALL/STOP_ALL, reply with `AlgorithmUpdateNotificationData`. Only bulk list
and paste requests use `AlgorithmListUpdateNotificationData`. Folder imports
always request Add, even when their source ID collides with a destination ID;
the core assigns a new folder ID.
