...
| `mt_algos_templates` | List the connected core's default per-type algorithm templates (the isConfigList broadcast the GUI's 'add new algorithm' dialog uses). These carry the core's current-version default argument set and are the clone source for mt_algos_create and mt_import_v2. Read-only. | — | — |
