  ...
  copy|export";
    ...
    public CommandResult Execute(string[] args)
    {
        ...
        return subCmd switch
        {
            ...
            "start" => AlgoAction(subArgs, AlgoOp.Start, targetProfile),
            "stop" => AlgoAction(subArgs, AlgoOp.Stop, targetProfile),
            "start-all" => AlgoAction(Array.Empty<string>(), AlgoOp.StartAll, targetProfile),
            "stop-all" => AlgoAction(Array.Empty<string>(), AlgoOp.StopAll, targetProfile),
            "save" => SaveAlgo(subArgs, start: false, targetProfile),
            "save-start" => SaveAlgo(subArgs, start: true, targetProfile),
    ...
    private CommandResult AlgoAction(string[] args, AlgoOp op, string? targetProfile)
    {
        CoreConnection? conn = ResolveConnection(targetProfile, out CommandResult? error);
        if (conn == null)
        {
        ...
        // Individual start/stop — now addressed purely by id (0.7.25267:
        // Algorithm{Run,Stop}RequestData carry just algorithmID, so the old
        // Serialize/Deserialize normalization and type-name resolution hacks
        // are obsolete — the core looks up its own stored config by id).
        if (op == AlgoOp.Start || op == AlgoOp.Stop)
        {
            string opName = op == AlgoOp.Start ? "START" : "STOP";
            if (args.Length < 1 || !long.TryParse(args[0], out long id))
            {
                return CommandResult.Fail($"Usage: algos {opName.ToLower()} <id>");
            }
            ...
            NotificationMessageData? notification = op == AlgoOp.Start
                ? conn.SendAlgorithmRun(id)
                : conn.SendAlgorithmStop(id);

            if (notification == null)
            {
                return CommandResult.Ok($"[{conn.Name}] Algorithm {id}: {opName} sent (response timed out).");
            }

            return notification.IsOk
                ? CommandResult.Ok($"[{conn.Name}] Algorithm {id} ({algo.name}): {opName} ✓",
                    new { Server = conn.Name, id, algo.name, Action = opName, notification.msgString })
                : CommandResult.Fail($"[{conn.Name}] Algorithm {id} ({algo.name}): {opName} FAILED — {notification.notificationCode}: {notification.msgString}");
        }
        else
        {
            string opName = op == AlgoOp.StartAll ? "START_ALL" : "STOP_ALL";
            NotificationMessageData? notification = op == AlgoOp.StartAll
                ? conn.SendAlgorithmRunAll()
                : conn.SendAlgorithmStopAll();

            if (notification == null)
            {
                return CommandResult.Ok($"[{conn.Name}] {opName} sent (response timed out).");
            }

            return notification.IsOk
                ? CommandResult.Ok($"[{conn.Name}] {opName} ✓",
                    new { Server = conn.Name, Action = opName, notification.msgString })
                : CommandResult.Fail($"[{conn.Name}] {opName} FAILED — {notification.notificationCode}: {notification.msgString}");
        }
    }
    ...
    private CommandResult SaveAlgo(string[] args, bool start, string? targetProfile)
    {
        CoreConnection? conn = ResolveConnection(targetProfile, out CommandResult? error);
        if (conn == null)
        {
        ...
        if (args.Length < 1 || !long.TryParse(args[0], out long id))
        {
            return CommandResult.Fail($"Usage: algos {(start ? "save-start" : "save")} <id>");
        }
        ...
        NotificationMessageData? notification = conn.SendAlgorithmSave(request, start: start);

        string? actionName = start ? "SAVE+START" : "SAVE";
    ...
    private CommandResult DeleteAlgo(string[] args, string? targetProfile, bool confirmed)
    {
        ...
        NotificationMessageData? notification = conn.SendAlgorithmDelete(id);
    ...
    private CommandResult ToggleDebug(string[] args, string? targetProfile)
    {
        ...
        NotificationMessageData? notification = conn.SendAlgorithmToggleDebug(id);
    ...
    private CommandResult CloneGroup(string[] args, string? targetProfile)
    {
        ...
        NotificationMessageData? notification = conn.SendFolderClone(groupId);
    ...
    /// <summary>
    /// Delete a folder (ex-group) and its algorithms. As of 0.7.25267 this is a
    /// dedicated AlgorithmFolderRemoveRequestData addressed by folder id.
    /// </summary>
    private CommandResult DeleteGroup(string[] args, string? targetProfile, bool confirmed)
    {
        ...
        NotificationMessageData? notification = conn.SendFolderDelete(groupId);
    ...
    private CommandResult CopyAlgo(string[] args, string? sourceProfile, bool confirmed)
    {
        ...
        NotificationMessageData? notification = destConn.SendAlgorithmSave(newAlgo);
    ...
    private CommandResult BulkEdit(string[] args, string? targetProfile, bool confirmed)
    {
        ...
        foreach (var entry in plan)
        {
            ...
            var request = new AlgorithmData(entry.AlgoRef);
            NotificationMessageData? notif = conn.SendAlgorithmSave(request, timeoutMs: 15_000);
            if (notif == null)
            {
                rows.Add(new { entry.AlgorithmId, entry.Name, entry.Signature,
    ...
    private CommandResult CreateAlgo(string[] args, string? targetProfile, bool confirmed)
    {
        ...
        // Commit path — dispatch via SendAlgorithmSave (same wire as paste).
        NotificationMessageData? notif = conn.SendAlgorithmSave(fresh, timeoutMs: 15_000);
        if (notif == null)
            return CommandResult.Ok(
    ...
    private CommandResult StartAndVerify(string[] args, string? targetProfile)
    {
        ...
        CommandResult startResult = AlgoAction(new[] { id.ToString() }, AlgoOp.Start, targetProfile);
